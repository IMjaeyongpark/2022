...
#include<windows.h>
#include<time.h>
#include<conio.h>		// getch(),kbhib() 함수를 사용
#define UP 72
#define DOWN 80
...
void printproto();				//가성베팅게임 경고 출력 함수
int keyControl();				//방향기 위와 아래를 인식하는 함수
void gotoxy(int x, int y);		//콘솔창 좌표 선택 함수
...
void  stringSplit(char data[]) {
	...
	for (i = 0; i < 20 && strcmp(analysis[i].team_name, homename) != 0; i++);	//경기한 팀 찾기
	score = strtok(NULL, "\t");
...
int select_menu() {
	...
	while (1) {
		...
		gotoxy(36, 1); printf("-----------------------------------\n");
		gotoxy(36, 2); printf("프리미어 리그 분석 및 가상 베팅게임\n");
		gotoxy(36, 3); printf("-----------------------------------\n");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);	//콘솔창 글씨 생상 변경
		if (select == 1)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
		gotoxy(40, 10); printf("1.시즌별 순위 출력\n");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);
		if (select == 2)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
		gotoxy(40, 11); printf("2.19~22시즌 팀별 정보 출력\n");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);
		if (select == 3)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
		gotoxy(40, 12); printf("3.가상 베팅 게임\n");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);
		if (select == 4)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
		gotoxy(40, 13); printf("4.종료\n\n");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);

		gotoxy(35, select + 9); printf("▶");
		key = keyControl();
		if (key == UP) {
...
void print_rank() {
	...
	while (1) {
		...
		gotoxy(49, 1); printf("---------");
		gotoxy(49, 2); printf("시즌 선택");
		gotoxy(49, 3); printf("---------");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);
		if (sel == 1)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
		gotoxy(48, 10); printf("1.19~20시즌");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);
		if (sel == 2)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
		gotoxy(48, 11); printf("2.20~21시즌");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);
		if (sel == 3)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
		gotoxy(48, 12); printf("3.21~22시즌");
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
	
		gotoxy(43, sel + 9); printf("▶");
		key = keyControl();
		if (key == UP) {
			if (sel == 1)sel = 3;
			else sel--;
	...
	rank = 1;
	gotoxy(40, 2); printf("<<%d~%d시즌 순위>>", sel + 18, sel + 19);
	gotoxy(27, 5); printf("순위\t팀\t승\t무\t패\t승점\t진출/강등\n");
	while (current != NULL) {		//선택한 시즌의 팀별 순위 및 정보 출력
		gotoxy(28, rank + 5); printf("%d\t%s\t%d\t%d\t%d\t%d", rank, current->team_name, current->win, current->draw, current->lose, current->point);
		if (rank < 5) {
...
void bigteam_analysis() {
	...
	while (1) {
		gotoxy(40, 1); printf("--------------");
		gotoxy(40, 2); printf("19~22시즌 전적");
		gotoxy(40, 3); printf("--------------");
		for (int i = 1; i < 16; i++) {
			gotoxy(45, i + 5); printf("%d:%s", i , bigteam[i-1].team_name);	//분석한 팀들의 이름 출력
		}
		gotoxy(40, 22); printf("팀 선택:");
		scanf("%d", &a);		//팀 선택
		if (a < 16 && a > 0)break;
		gotoxy(40, 22); printf("다시 입력하세요!!\n\n\n\n");
		system("pause");
		system("cls");
	}
	a--;
...
void proto() {
	...
	USER* betgame = NULL, * new = NULL, * cur = NULL, * che = NULL;			//사용자가 선택한 경기를 저장하는 연결리스트
	...
	while (1) {
		...
		if (plus == -1) {
			system("cls");
			gotoxy(40, 10); printf("손익:");
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 4);
			if (profit >= 0)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 2);
			printf(" %d 원\n\n\n\n\n", profit);
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
			return;
		}
		...
		while (1) {
			...
			gotoxy(30, 7); printf(" home\t\t\taway\n\n");
			current = list;
			che = betgame;
			while (che != NULL) {
				if (che->bett == 1)gotoxy(27, che->gam + 8);
				else gotoxy(60, che->gam + 8);
				printf("승");
				che = che->next;
			}
			for (int k = 1; current != NULL; k++) {		//가상경기의 정보 출력
				if (game[k - 1] == 1)SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 8);
				gotoxy(30, k + 8); printf("%d.%s\t%.2f:%.2f\t%s\n", k, bigteam[current->home].team_name, current->h_bet, current->a_bet, bigteam[current->away]);
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
				current = current->next;
			}
			printf("\n배팅할 금액 선택:");
		...
		for (int k = 0; k < gamecount && selgame != -1; k++) {
			...
			while (1) {
				...
				if (selgame > gamecount || selgame < -1 || selgame == 0) {
					printf("\n\n다시 입력하세요!!\n\n\n");
					system("pause");
					system("cls");
				}
				else if (game[selgame - 1] == 1) {
					printf("\n\n이미 선택한 경기입니다!\n\n\n");
					system("pause");
					system("cls");
				}
		...
		if (checkwin == 0) {		//사용자의 베팅에 적중여부
			...
			printf("\t\t\t\t\t\t승리!!\n");
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
			printf("\t\t\t\t\t    %d원 획득", (int)((float)betm * bet));
...
int* playgame(USER* node) {
	...
	for (int i = 0; i < (int)node->betwin; i++)random[i] = 1;		//사용자가 선택한 팀의 승률만큼 배열에 1삽입
	while (1) {
		score[0] = 0;
		score[1] = 0;
		for (int i = 0; i < rand() % 6 + 1; i++) {
			if (random[rand() % 100] == 1)score[node->bett - 1]++;
			else {
				if (node->bett == 1)score[1]++;
				else score[0]++;
			}
		}
...
void gotoxy(int x, int y) {
	...
	Pos.X = x;
	Pos.Y = y;
	SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), Pos);		//콘솔창 좌표 이동
}